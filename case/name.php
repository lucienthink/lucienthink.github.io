...
if ('score'==$_REQUEST['m']){
  ...
  //格字 简略内容
    ...
    for($i=1;$i<13;$i++)
    { 
      $result_12 .= strip_tags($info1[0][$i])."<br>";
    }
    //var_dump($result_12);
    $_SESSION['result_12'] = $result_12;
  //格 字的详细内容
    ...
    for($i=5;$i<10;$i++)
  {
      ...
      $start=strpos($tmp,$info2[0][$i]);
      $tmp=substr($tmp,$start);

      if($i == 9)
      $end=strpos($tmp,"function");
      else
      $end=strpos($tmp,$info2[0][$i+1]);  

      $tmp=substr($tmp,0,$end-1);
      ...
      $arr1 = array('<ul id="name_title" style="background:#f5f5f5;">','</ul>','name_tabs" style="border-top:0px;">','<script language="javascript">');
      $tmp = str_replace($arr, "<br>", $tmp);
      $tmp = str_replace($arr1, '', $tmp);
      $tmp = str_replace('</li>', '</h1>', $tmp);
      $tmp = str_replace('<li>', '<h1>', $tmp);
      $tmp = str_replace('li>', '<h1>', $tmp);
      $_SESSION['ge'][$i-5] = $tmp;
      //var_dump($tmp);
...
//得分
  ...
  //繁体字
   ...
   // var_dump($info3);
    ...
    $_SESSION['zi'][0] = $info3[0][0].$info3[0][1].'<br><br>'.$info3[0][2].$info3[0][3];
